...
class HistoricalAppender:
    ...
    def __init__(self, max_workers: int = 5):
        ...
        self.data_collector = MultiSourceDataCollector()  # Use enhanced multi-source collector
    ...
    def fetch_symbol_data(self, symbol: str, period: str = "max", interval: str = "1d") -> Optional[pd.DataFrame]:
        """Fetch data for a single symbol using enhanced multi-source collector"""
        try:
            logger.info(f"📥 Fetching {symbol} {interval} data (period: {period})")
            
            # Use enhanced multi-source collector (Alpha Vantage priority)
            data, source = self.data_collector.fetch_data(symbol, period, interval)
            ...
            logger.info(f"✅ Fetched {symbol} {interval} from {source}: {len(data)} rows from {data.index.min()} to {data.index.max()}")
            return data
