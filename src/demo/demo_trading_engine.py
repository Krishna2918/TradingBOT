...
class DemoTradingEngine:
    ...
    def generate_ai_signals(self) -> List[Dict]:
        ...
        if self.ai_enabled and self.ai_ensemble:
            try:
                ...
                # Check if AI ensemble actually produced valid results
                if ensemble_decision.get('confidence', 0) == 0 or 'No AI analysis available' in ensemble_decision.get('reason', ''):
                    logger.warning(" AI ensemble returned no valid analysis, using fallback")
                    signals = self._generate_fallback_signals()
                # Convert ensemble decision to trading signals
                elif ensemble_decision['signal'] in ['buy', 'sell'] and ensemble_decision['confidence'] > 0.6:
                    ...
                    # AI says hold - still generate some fallback signals for demo activity
                    logger.info(f" AI Ensemble: {ensemble_decision['signal']} - supplementing with demo signals")
                    signals = self._generate_fallback_signals()
    ...
    def _generate_fallback_signals(self) -> List[Dict]:
        ...
        for symbol in self.symbols[:10]:  # Focus on top 10 for demo
            ...
            # Generate signals for demo mode (higher probability for testing)
            if random.random() < 0.40:  # 40% chance per symbol for active demo
