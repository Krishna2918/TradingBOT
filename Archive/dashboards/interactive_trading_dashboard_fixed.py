...
# Add JavaScript debugging - this will log to browser console
app.clientside_callback(
    """
    function(n_clicks) {
        console.log("🔥 CLIENT-SIDE: Button clicked!", n_clicks);
        if (n_clicks) {
            console.log("✅ CLIENT-SIDE: Callback is working!");
        }
        return window.dash_clientside.no_update;
    }
    """,
    Output('debug-test-output', 'data'),
    Input('debug-test-btn', 'n_clicks'),
...
app.layout = html.Div([
    ...
    dcc.Store(id='debug-test-output', data=''),
    
    # DEBUG: Test button at top of page (always visible)
    html.Div([
        dbc.Button("DEBUG TEST BUTTON", id="debug-test-btn", color="danger", className="m-2"),
        html.Div(id="debug-test-result", style={'color': 'yellow', 'padding': '10px'})
...
# DEBUG TEST CALLBACK - Tests if ANY callbacks work
@app.callback(
    Output('debug-test-result', 'children'),
    Input('debug-test-btn', 'n_clicks'),
    prevent_initial_call=True
)
def debug_test_callback(n_clicks):
    """Simple test to verify Dash callbacks are working"""
...
def initialize_trading(n_clicks, starting_capital, initialized):
    ...
    if not n_clicks:
        print("⚠️ No clicks detected")
        return (False, no_update)
    
    if not starting_capital:
        print("⚠️ No starting capital provided")
        return (False, no_update)
    
    if starting_capital < 10:
        print(f"⚠️ Capital too low: {starting_capital} < 10")
        return (False, no_update)
    ...
    if n_clicks and starting_capital and (starting_capital >= 10):
        ...
        # Return success and trigger dashboard display
        print("✅ TRADING INITIALIZED SUCCESSFULLY!")
        print("Returning to display dashboard...")
        
        # Simple test to see if callback is working
        test_content = html.Div([
            html.H1("IT WORKS!", style={'color': 'green', 'textAlign': 'center'}),
            html.H3(f"Capital: ${starting_capital}", style={'textAlign': 'center'}),
            html.P("If you see this, the callback is firing!", style={'textAlign': 'center'})
        ])
...
# Removed duplicate render_router function below
    try:
        if pathname == '/logs':
            return (create_ai_logs_page(), True, True, trading_state.get('initialized', False))
        if not trading_state.get('initialized', False):
...
def switch_log_tab(activity_clicks, trades_clicks, signals_clicks, decisions_clicks):
    ...
    button_id = ctx.triggered[0]['prop_id'].split('.')[0]
    tab_map = {
        'log-tab-activity': 'activity',
        'log-tab-trades': 'trades',
        'log-tab-signals': 'signals',
        'log-tab-decisions': 'decisions'
    }
...
if __name__ == '__main__':
    ...
    try:
        app.run(
            debug=True,  # Enable debug mode to see callback errors
            host='127.0.0.1', 
            port=8051,
            dev_tools_hot_reload=False  # Disable hot reload (can cause issues)
        )
    except Exception as e:
